...
class Detic(CascadeRCNN):
    ...
    def __init__(self,
        ...
        self.out_pkl_folder = out_pkl_folder
    ...
    def predict(self,
        ...
        if self.save_dets:
            # result = batch_data_samples
            for result in batch_data_samples:
                res_dict = {}
                res_dict['det_bboxes'] = torch.cat([result.pred_instances.bboxes, result.pred_instances.scores.unsqueeze(1)], dim=1).to('cpu')
                res_dict['det_labels'] = result.pred_instances.labels.to('cpu')
                if 'masks' in result.pred_instances:
                    res_dict['det_masks'] = encode_mask_results(result.pred_instances.masks.to('cpu').numpy())

                pkl_name = result.img_path.replace('data/tao/frames/val/','').replace('.jpg','.pth')
                pkl_path = os.path.join(self.out_pkl_folder, pkl_name)
                dir_name = os.path.dirname(pkl_path)
                os.makedirs(dir_name, exist_ok=True)
                pickle.dump(res_dict, open(pkl_path, 'wb'))
